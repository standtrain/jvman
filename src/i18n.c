...
static const JvmanTranslation translations[] = {
    ...
    {"Usage:", "使用方法："},
    {"  jvman install <major> [--name <name>] [--sha256 <hex>] [--source <name>] [--no-persist]",
     "  jvman install <主版本> [--name <名称>] [--sha256 <校验值>] [--source <下载源>] [--no-persist]"},
    {"  jvman install <name> --archive <file> [--sha256 <hex>] [--no-persist]",
     "  jvman install <名称> --archive <文件> [--sha256 <校验值>] [--no-persist]"},
    {"  jvman add <name> <jdk-home>", "  jvman add <名称> <JDK主目录>"},
    {"  jvman use <name> [--no-persist]", "  jvman use <名称> [--no-persist]"},
    {"  jvman activate [--replace-java-home]",
     "  jvman activate [--replace-java-home]"},
    {"  jvman deactivate", "  jvman deactivate"},
    ...
    {"  jvman update [--check] [--version <version>] [--source <name>|--source-list]",
    ...
    {"the current shell environment cannot be activated safely in CMD",
     "当前 shell 环境包含无法在 CMD 中安全激活的字符"},
    {"the current shell PATH is too long for CMD",
     "当前 shell 的 PATH 过长，无法通过 CMD 激活"},
    ...
    {"Registered %s -> %s\n", "已注册 %s -> %s\n"},
    {"Now using %s (%s)\n", "当前使用 %s（%s）\n"},
    {"The selected Java is not active in this shell. A child process cannot update its parent shell. Initialize this shell once with one of:\n",
     "当前终端尚未激活所选 Java。子进程无法修改父终端环境，请在本终端执行一次以下任一初始化命令：\n"},
    {"  CMD: for /f \"delims=\" %%L in ('jvman init cmd') do @%%L\n",
     "  CMD: for /f \"delims=\" %%L in ('jvman init cmd') do @%%L\n"},
    {"  PowerShell: jvman init powershell | Invoke-Expression\n",
     "  PowerShell: jvman init powershell | Invoke-Expression\n"},
    {"The selected Java is not active in this shell. A child process cannot update its parent shell. Initialize this shell once with:\n",
     "当前终端尚未激活所选 Java。子进程无法修改父终端环境，请在本终端执行一次以下初始化命令：\n"},
    {"  sh: eval \"$(jvman init sh)\"\n",
     "  sh: eval \"$(jvman init sh)\"\n"},
    {"Add the same initialization to your shell startup file for future terminals.\n",
     "将同一初始化命令加入 shell 启动配置后，后续终端会自动生效。\n"},
    {"Persistent activation is already in place; new terminals and IDEs will pick up the change.\n",
     "已完成持久激活；新终端与 IDE 将自动生效。\n"},
    {"To refresh the current shell without reopening, run:\n",
     "如需刷新当前终端而无需重新打开，请执行：\n"},
    {"Persistent activation has been applied. New terminals and IDEs will pick up the change.",
     "已完成持久激活。新终端与 IDE 将自动生效。"},
    {"Persistent activation is not managed by jvman; nothing to remove.",
     "当前系统中的持久激活并非由 jvman 管理，无需移除。"},
    {"Removed persistent activation.", "已移除持久激活。"},
    {"jvman: cannot read installer metadata (status %d)\n",
     "jvman：无法读取安装器元数据（状态 %d）\n"},
    {"jvman: cannot update HKCU Path (status %d)\n",
     "jvman：无法更新 HKCU Path（状态 %d）\n"},
    {"jvman: cannot update HKCU JAVA_HOME (status %d)\n",
     "jvman：无法更新 HKCU JAVA_HOME（状态 %d）\n"},
    {"jvman: cannot persist installer metadata (status %d)\n",
     "jvman：无法持久化安装器元数据（状态 %d）\n"},
    {"jvman: cannot restore HKCU JAVA_HOME (status %d)\n",
     "jvman：无法恢复 HKCU JAVA_HOME（状态 %d）\n"},
    {"jvman: cannot remove <current>/bin from HKCU Path (status %d)\n",
     "jvman：无法从 HKCU Path 中移除 <current>/bin（状态 %d）\n"},
    {"jvman: cannot update installer metadata (status %d)\n",
     "jvman：无法更新安装器元数据（状态 %d）\n"},
    {"jvman: JAVA_HOME already set to a different value; pass --replace-java-home to override or run `jvman deactivate` first.\n",
     "jvman：JAVA_HOME 已存在其他值；请使用 --replace-java-home 覆盖，或先执行 `jvman deactivate`。\n"},
    {"jvman: cannot compute current/bin path\n",
     "jvman：无法计算 current/bin 路径\n"},
    {"jvman: cannot convert install paths to UTF-16 for HKCU\n",
     "jvman：无法将安装路径转换为 UTF-16 以写入 HKCU\n"},
    {"jvman: out of memory preparing metadata\n",
     "jvman：准备元数据时内存不足\n"},
    {"jvman: cannot convert current/bin path\n",
     "jvman：无法转换 current/bin 路径\n"},
    {"jvman: no shell rc files were updated (missing $HOME or targets)\n",
     "jvman：未更新任何 shell rc 文件（缺少 $HOME 或目标）\n"},
    {"jvman: cannot update %s\n", "jvman：无法更新 %s\n"},
    {"machine-wide activation is not implemented in the CLI; use jvman-setup.exe",
     "CLI 尚未实现机器范围激活；请使用 jvman-setup.exe"},
    {"usage: jvman use <name> [--no-persist]",
     "用法：jvman use <名称> [--no-persist]"},
    {"usage: jvman activate [--replace-java-home]",
     "用法：jvman activate [--replace-java-home]"},
    {"usage: jvman deactivate", "用法：jvman deactivate"},
    {"  (no JDKs registered)\n", "  （未注册 JDK）\n"},
    ...
    {"Update sources:", "更新源："},
    {"benchmark all mirrors and pick the fastest",
     "对所有镜像测速并选最快"},
    {"Testing update mirrors...", "正在测试更新镜像..."},
    {"  %-8s %llu ms\n", "  %-8s %llu 毫秒\n"},
    {"  %-8s %lu ms\n", "  %-8s %lu 毫秒\n"},
    {"  %-8s %llu ms, unavailable (%s)\n",
     "  %-8s %llu 毫秒，不可用（%s）\n"},
    {"  %-8s %lu ms, unavailable (%s)\n",
     "  %-8s %lu 毫秒，不可用（%s）\n"},
    {"  %-8s %s\n", "  %-8s %s\n"},
    {"Selected update mirror: %s (%llu ms)\n",
     "已选择更新镜像：%s（%llu 毫秒）\n"},
    {"Selected update mirror: %s (%lu ms)\n",
     "已选择更新镜像：%s（%lu 毫秒）\n"},
    {"unreachable", "无法访问"},
    {"no update mirror is reachable", "没有可用的更新镜像"},
    {"cannot construct the fixed release URL",
     "无法构造发布地址"},
    {"unknown update mirror; use `jvman update --source-list`",
     "未知的更新镜像；请运行 `jvman update --source-list` 查看可用镜像"},
    {"update option --source requires one mirror name",
     "--source 需要一个镜像名"},
    {"usage: jvman update [--check] [--version <version>] [--source <name>|--source-list]",
     "用法：jvman update [--check] [--version <版本>] [--source <名称>|--source-list]"},
    {"Custom: %s", "自定义：%s"},
